...
create table if not exists public.invoices (
  ...
  tx_id text,
  kind text not null check (kind in ('standard', 'subscription', 'multipay')),
  status text not null check (status in ('draft', 'pending_signature', 'pending_chain', 'paid', 'expired', 'failed')) default 'draft',
